...
SELECT v.name, v.author_id, v.description,
...
) AS v(name, author_id, description, ingredients, instructions, steps,
       prep_time_minutes, cook_time_minutes, servings,
       tags, cooking_method, flavor, ingredient_type, status, nutrition_info)
WHERE NOT EXISTS (
    SELECT 1 FROM recipes r WHERE r.name = v.name AND r.author_id = v.author_id
);
...
JOIN recipes r ON r.name = d.recipe_name AND r.author_id = 1
...
CROSS JOIN (SELECT id FROM users WHERE username = 'testuser') u
LEFT JOIN recipes r ON r.name = d.recipe_name AND r.author_id = 1
WHERE NOT EXISTS (
